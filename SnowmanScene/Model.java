...
public class Model {
  ...
  // supply offsets for moving background texture
  public void setBackground(GL3 gl, double startTime) {
    Mat4 modelMatrix = new Mat4(1);
    elapsedTime = getSeconds() - startTime;
    t = elapsedTime*0.1;
    offsetY = (float)(t - Math.floor(t));
    offsetX = (float)(Math.sin(elapsedTime*2)*0.09);
    mBackgorund(offsetX, offsetY, "offset1", gl);
    offsetX = (float)(t - Math.floor(t));
    offsetY = (float)(Math.sin(elapsedTime*0.9)*0.05);
    mBackgorund(offsetX, offsetY, "offset2", gl);
    offsetX = (float)(Math.sin(elapsedTime*0.8)*0.09) - (float)t;
    offsetY = (float)(Math.sin(elapsedTime*0.8)*0.09) + (float)t;
    mBackgorund(offsetX, offsetY, "offset3", gl);
  }

  // set textures for shader
  public void mBackgorund(float x, float y, String offset, GL3 gl) {
    shader.use(gl);
    shader.setFloat(gl, offset, x, y);
  ...
  public void render(GL3 gl, Mat4 modelMatrix) {
    ...
    shader.setVec3(gl, "light.sunlight", light.getSunlight());
    shader.setFloat(gl, "light.cutOff", light.getCutOff());
